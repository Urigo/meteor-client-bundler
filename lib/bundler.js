...
function bundle(options) {
  ...
  if (config.externalNpmPackages) {
      ...
      injectedData += `
        meteorInstall({
          'node_modules': {
            '${packageName}': {
              'index.js'(e, r, m){
                m.exports = require('${packageName}');
              },
              'package.json'(e,r,m){
                m.exports = require('${packageName}/package.json');
              }
            }
          }
        });
    ...
    const replacedText = `Package._define("modules-runtime", {`;
    const result = data.replace(replacedText, injectedData + replacedText);
